    ...
    <div class="card">
        ...
        <div class="row card-header flex-column flex-md-row pb-0 mb-5">
            <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto mt-0">
                <h5 class="card-title mb-0 text-md-start text-center">Daftar Instansi</h5>
            </div>
            <div class="d-md-flex justify-content-between align-items-center dt-layout-end col-md-auto ms-auto mt-0">
                <button class="btn btn-primary mb-3 " data-bs-toggle="modal" data-bs-target="#addAgendaModal">Tambah
                    Instansi</button>
            </div>
        </div>
                ...
                <tbody id="tabel-agenda">
                    @foreach ($instansi as $item)
                        <tr>
                            ...
                            <td>{{ $item->periode_pengirim }}</td>
                            <td>
                                <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#editAgendaModal" data-id="{{ $item->id_instansi }}">Ubah</button>

                                <form action="{{ route('agenda.destroy', $item->id_instansi) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal fade" id="addAgendaModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form id="formAddSuratMasuk" action="{{ route('instansi.store') }}" method="POST"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#editAgendaModal').on('show.bs.modal', function(event) {
                ...
                // Ambil data via AJAX
                $.get('/instansi/' + id + '/edit', function(data) {
        ...
        function refreshTable(response, token) {
            ...
            $.each(response.data.instansis, function(index, obj) {
                $('#tabel-agenda').append('' +
                    ...
                    ' <form action="/instansi/' + obj.id_instansi + '" method="POST"' +
                    ' class="d-inline">' +
